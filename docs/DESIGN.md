...
| 8 | `feat: forms with React Hook Form and Zod` | Controlled inputs, schema validation, multi-step forms |
| 9 | `feat: animations, bottom sheet, and modals` | Reanimated 3, gesture handlers, @gorhom/bottom-sheet, modal/action sheet patterns |
| 10 | `feat: realtime with WebSocket, Socket.IO, and AppState` | WS lifecycle, Zustand real-time store, reconnect on foreground, AppState API |
| 11 | `feat: webview with JS bridge and OAuth flow` | react-native-webview, postMessage bridge, in-app browser, OAuth redirect handling |
| 12 | `test: unit, component, and integration tests` | RNTL setup, store tests, screen integration tests |
| 13 | `ci: GitHub Actions and Fastlane lanes` | ci.yml (lint, typecheck, tests, coverage gate, audit, bundle size), staging.yml + release.yml (Fastlane build → GitHub artifact), Sentry source maps upload, OTA via code-push-server |
| 14 | `dx: code snippets and feature generator` | Plop.js scaffolding, VS Code snippets, CLAUDE.md conventions |
| 15 | `test: Detox E2E for auth and navigation` | Detox config, device/emulator tests, CI E2E job |
...
| 32 | `dx: code-gen Claude Code skill` | Custom Claude Code skill that accepts a task description, produces a mini design (screen name, props, navigation wiring), scaffolds the screen using ScreenWrapper + project conventions, runs self-review against the design, and commits + pushes to git |
